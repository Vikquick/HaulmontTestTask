...
public class OrderListener {
 public Window orderAddWindow (Grid grid) {
    ...
    HorizontalLayout buttonsLayout = new HorizontalLayout();
     ...
     for (int i = 0; i < index; i++) {clientsID.add(clients.get(i).getId());}
    ...
    void deleteOrder (Grid grid) {
        ...
        {Notification.show("Выберите заказ для удаления");}
            ...
            try {DBConnection.startConnection();
        ...
        catch (java.lang.NullPointerException e){Notification.show("Выберите заказ для удаления");}
        }
 }
